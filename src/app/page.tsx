...
export default async function HomePage({searchParams}: Props) {
  ...
  const dogBreeds = isResponseTActionsGenericResponse(breedsData[1]) ? [] : breedsData[1]
  const catBreeds = isResponseTActionsGenericResponse(breedsData[0]) ? [] : breedsData[0]

  const breeds = shuffleTwoArrays(dogBreeds, catBreeds) as ICatDogResponse[]
  const filteredBreeds = search
    ? breeds.filter(el => el.breeds.some(breed => breed.name.toLocaleLowerCase().includes(search.toLocaleLowerCase())))
    : breeds

  return (
    <div className="flex flex-col gap-6">
<BreedSearch />
    <div className="grid grid-cols-2 gap-2 md:grid-cols-3 grid-auto-columns-1/2 md:grid-auto-columns-1/3">
